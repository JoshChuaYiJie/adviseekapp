...
export const CommunitySidebar = () => {
  ...
  return (
    <div className="w-64 h-full bg-background border-r border-border p-4 flex flex-col gap-4">
      <div className="mb-4">
        <h2 className="text-xl font-bold">{t('community.title', 'Community')}</h2>
        <p className="text-sm text-muted-foreground">{t('community.join_discussions', 'Join discussions with other students')}</p>
      </div>
      
      <div className="relative mb-4">
        <Search className="absolute left-2 top-2.5 h-4 w-4 text-muted-foreground" />
        <Input placeholder={t('community.search', 'Search discussions')} className="pl-8" />
      </div>
