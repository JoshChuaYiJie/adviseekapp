...
export const SegmentAdviseekChat = ({
  segmentType
}: SegmentAdviseekChatProps) => {
  ...
  return <div className="w-full">
      ...
      {isOpen && <Card className="mt-4 p-4">
          <h4 className="font-medium mb-2">Adviseek Assistant: {segmentType}</h4>
          
          <ScrollArea className="h-60 mb-4 border rounded-md p-2">
            <div className="space-y-3">
              {messages.map((msg, i) => <div key={i} className={`p-3 rounded-lg ${msg.role === "assistant" ? "bg-muted text-foreground mr-8" : "bg-primary/10 ml-8"}`}>
          ...
          <div className="flex gap-2">
            <Textarea value={input} onChange={e => setInput(e.target.value)} placeholder={`Ask about your ${segmentType.toLowerCase()} section...`} className="flex-1 resize-none" disabled={loading} onKeyDown={e => {
